...
public class GPTE4I2 : MonoBehaviour
{
    ...
    private string systemPrompt = "你是一位中文学习助手。你将评价学生对图片的描述并引导他们。图片显示两个男孩在走廊内踢球，女孩在窗口看着他们。\n\n你的任务是：\n1. 评价学生的回答\n2. 如果回答不够具体或不完全正确，提供引导性提示，帮助学生思考出确切的描述。例如：\n   - 如果学生没有提到具体地点，可以引导：'请再仔细观察图片，他们是在什么场所踢球？'\n   - 如果学生提到室内但不具体，可以引导：'你能更具体地描述一下他们现在踢球的地方吗？'\n3. 如果学生回答包含以下关键词之一，才判定为正确：'操场'、'草坪'、'室外'，然后告诉他们'回答正确，请继续在场景中探索'\n\n重要：\n- 你的回答必须是纯文本\n- 不接受模糊或一般性的答案，如'外面'等，这些都不够具体\n- 引导学生思考图片中的线索，但不直接提供答案\n- 只有当学生明确提到表示操场或草坪的词语时，才判定为正确";
    ...
    void Start()
    {
        ...
        // 添加初始问题
        messages.Add(new ChatMessage { role = "assistant", content = "小男孩们现在在那里踢球？" });
            ...
            else
            {
                ...
                bool isCorrect = content.Contains("正确") && content.Contains("继续") &&
                    (content.Contains("回答正确") ||
                     studentResponse.text.Contains("操场") ||
                     studentResponse.text.Contains("草坪") ||
                     studentResponse.text.Contains("室外"));
    ...
    void EndConversation(bool isCorrect)
    {
        ...
        {
            // 提供标准答案示例
            string standardAnswer = "经过刚才的对话，男孩们去了楼下操场踢球";
