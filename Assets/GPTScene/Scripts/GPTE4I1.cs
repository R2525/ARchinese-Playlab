...
public class GPTE4I1 : MonoBehaviour
{
    [Header("API")]
    ...
    private string systemPrompt = "你是一位中文学习助手。你将评价学生对图片的描述并引导他们。图片显示一个小女孩在窗边，看到两个男孩在走廊上，他们似乎不再踢球了，小女孩表情欣慰。\n\n你的任务是：\n1. 评价学生的回答\n2. 如果回答不够具体或不完全正确，提供引导性提示，帮助学生思考出确切的描述。例如：\n   - 如果学生只说'小女孩看起来很高兴'或类似模糊答案，引导他们更具体地描述表情：'你能更准确地描述一下小女孩的表情吗？是单纯的高兴，还是有其他情感？'\n   - 如果学生提到'表情'但没有描述其特性，可以引导：'小女孩的表情似乎表明她对看到的场景有特定的感受，你能更具体地描述这种表情吗？'\n3. 如果学生回答包含以下关键词之一，才判定为正确：'欣慰'、'释然'、'放心'、'安心'、'宽慰'，然后告诉他们'回答正确，请继续在场景中探索'\n\n重要：\n- 你的回答必须是纯文本\n- 不接受模糊或一般性的答案，如'高兴'、'开心'等，这些都不够具体\n- 引导学生思考表情背后的情感和原因，但不直接提供答案\n- 只有当学生明确提到表示欣慰性质的词语时，才判定为正确";
    ...
    void Start()
    {
        ...
        // 添加初始问题
        messages.Add(new ChatMessage { role = "assistant", content = "女孩脸上是什么表情？" });
            ...
            else
            {
                ...
                bool isCorrect = content.Contains("正确") && content.Contains("继续") &&
                    (content.Contains("回答正确") ||
                     studentResponse.text.Contains("欣慰") ||
                     studentResponse.text.Contains("释然") ||
                     studentResponse.text.Contains("放心") ||
                     studentResponse.text.Contains("安心") ||
                     studentResponse.text.Contains("宽慰"));
    ...
    void EndConversation(bool isCorrect)
    {
        ...
        {
            // 提供标准答案示例
            string standardAnswer = "看见男孩们没有在走廊上踢球了，小女孩露出了欣慰的表情。";
